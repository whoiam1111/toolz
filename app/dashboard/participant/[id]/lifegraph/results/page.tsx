...
export default function SixTypeResultsPage() {
    ...
    return (
        <div className="max-w-5xl mx-auto mt-10 px-4 sm:px-6 lg:px-8 py-10 bg-white rounded-3xl shadow-lg space-y-10">
            <h1 className="text-4xl font-extrabold text-center text-indigo-700">인생그래프 검사 결과</h1>

            {imageUrl ? (
                <div className="w-full flex justify-center">
                    <div className="relative w-full aspect-[1.414/1] max-w-5xl bg-gray-50 rounded-2xl border border-indigo-200 shadow-xl overflow-hidden">
                        <Image src={imageUrl} alt="인생그래프 이미지" fill className="object-contain" priority />
                    </div>
                </div>
            ) : (
